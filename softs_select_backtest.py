"""
softs_select_backtest.py
────────────────────────
ZF-only rates + Cocoa/Coffee only (drop ZW, ZS, ZC, SB).

Based on finding that full softs (6-instrument) were mixed:
  - Cocoa/Coffee historically stronger trend instruments
  - ZW/ZS/ZC/SB added noise in 2017–2022

Rates: ZF only, B=D50 C=D100
Softs: KC + CC only (Coffee + Cocoa)
"""
...
ENERGY      = ["CL", "NG"]
AGRICULTURE = ["KC", "CC"]                    # Coffee + Cocoa only
...
BASELINES = {
    ...
    "ZF-only (full agri)": {"cagr": 0.1077, "max_drawdown": -0.2923, "mar_ratio": 0.369, "sharpe": 0.55, "longest_underwater_days": 999},
}
...
t0 = time.time()
print("=" * W)
print("  ZF-ONLY RATES + COCOA/COFFEE ONLY")
print("  Rates  : ZF only,  B=D50  C=D100")
print("  Softs  : KC (Coffee) + CC (Cocoa) only — ZW/ZS/ZC/SB excluded")
print("  Metals : B=D50  C=D100  |  Equity: B=D75  C=D150")
...
print(f"\n{'═'*W}")
print(f"  PORTFOLIO METRICS — 2008–2025  ZF-only + KC/CC softs")
print(f"{'═'*W}")
