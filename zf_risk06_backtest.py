"""
zf_risk06_backtest.py
─────────────────────
ZF-only baseline with risk reduced from 0.7% to 0.6% per trade.
All other settings identical to the ZF-only leader config.
"""
...
RISK_PER_TRADE = 0.006        # ← 0.6% (was 0.7%)
GLOBAL_CAP     = 6.0
START, END     = "2008-01-01", "2025-12-31"
W = 72

BASELINES = {
    "ZF-only 0.7%": {"cagr": 0.1077, "max_drawdown": -0.2923, "mar_ratio": 0.369, "sharpe": 0.55, "longest_underwater_days": 999},
}
...
t0 = time.time()
print("=" * W)
print("  ZF-ONLY BASELINE  —  0.6% RISK PER TRADE")
...
print(f"\n{'═'*W}")
print(f"  PORTFOLIO METRICS — 2008–2025  (0.6% risk)")
print(f"{'═'*W}")
...
col_w  = 18
hdr = f"  {'Metric':<12}  {'0.6% run':>10}  {'0.7% baseline':>{col_w}}  {'Delta':>{col_w}}"
print(hdr)
print("  " + "─" * (len(hdr) - 2))
for lbl, key, val, fmt in metrics_rows:
    bv   = BASELINES["ZF-only 0.7%"][key]
    d    = val - bv
    sign = "+" if d >= 0 else ""
